...
class cameraComponent extends Component<Props> {
    ...
    takePicture = async () => {
        if (this.camera) {
          if (Platform.OS === 'android') {
            ...
            //save photo
            CameraRoll.save(data.uri, {type: 'photo', album: 'Voc Photos'}).then(onfulfilled => {
                ToastAndroid.show(`Voc Photos: ${onfulfilled}`, ToastAndroid.SHORT);
            }).catch(error => {
  ...
  startRecording = async () => {
    ...
    CameraRoll.save(data.uri, {type: 'video', album: 'Voc Videos'}).then(onfulfilled => {
        ToastAndroid.show(`Voc Videos: ${onfulfilled}`, ToastAndroid.SHORT)
